...
void WorkStealingThreadPool::ThreadState::ThreadBody() {
  ...
  if (pool_->IsForking()) {
    ...
    while (!g_local_queue->Empty()) {
      closure = g_local_queue->PopMostRecent(/*stealing=*/false);
      if (closure != nullptr) {
...
bool WorkStealingThreadPool::ThreadState::Step() {
  ...
  auto* closure = g_local_queue->PopMostRecent(/*stealing=*/false);
  ...
  while (!pool_->IsForking()) {
    ...
    closure = pool_->queue()->PopMostRecent(/*stealing=*/false);
    if (closure != nullptr) {
...
void WorkStealingThreadPool::ThreadState::FinishDraining() {
  ...
  while (!pool_->IsForking()) {
    ...
    if (!pool_->queue()->Empty()) {
      auto* closure = pool_->queue()->PopMostRecent(/*stealing=*/false);
      if (closure != nullptr) {
        closure->Run();
      }
      continue;
